...
def parse_memory_profiling_data(filepath):
    ...
    patterns = {
        "memory_allocated_in_heap": (
            r"(\d+(?:,\d+)*) bytes allocated in the heap",
            "B",
        ),
        ...
        for line in file:
            for key, (pattern, unit) in patterns.items():
                ...
                if match:
                    ...
                    if (
                        key == "memory_maximum_slop"
                    ):  # Convert maximum slo to KiB and truncate
                        value //= 1024
                        unit = "KiB"
...
def subdict(original_dict, keys_to_extract):
    if keys_to_extract is None:
        ...
        return {
            key: original_dict[key] for key in keys_to_extract if key in original_dict
        }
...
def convert_dict_to_list(data, keys_to_extract=None):
    return [
        {"name": name, **details}
        for name, details in subdict(data, keys_to_extract).items()
    ]


def save_github_action_benchmark_json(
    output_path, benchmarks, memory_stats, benchmark_keys, memory_keys
):
    with open(output_path, "w") as file:
        json.dump(
            convert_dict_to_list(benchmarks, benchmark_keys)
            + convert_dict_to_list(memory_stats, memory_keys),
            file,
            indent=2,
        )
    ...
    def custom_sort(item):
        ...
        if is_not_seconds_unit:
            ...
            return (
                True,
                item["name"][0].islower(),
                0 if commit_hash not in item.keys() else -float(item[commit_hash]),
            )
...
if __name__ == "__main__":
    ...
    save_github_action_benchmark_json(
